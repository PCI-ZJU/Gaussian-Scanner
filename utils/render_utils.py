...
def save_depth_colored(
    ...
    vmax=None,
    percentile=None,
    invalidate_le_zero=True,
    invert: bool = False,
    with_alpha: bool = True,
):
  d = np.asarray(depthmap).astype(np.float32)
  mask = ~np.isfinite(d)
  if invalidate_le_zero:
    mask |= (d<=0)
  
  valid = d[~mask]
  if valid.size == 0:
    raise ValueError("None depth valid")
  if percentile is not None:
    lo, hi = np.percentile(valid, percentile)
    if vmin is None: vmin = float(lo)
    if vmax is None: vmax = float(hi)
  if vmin is None: vmin = float(valid.min())
  if vmax is None: vmax = float(valid.max())
  ...
  if with_alpha:
    ...
    rbg = rgba[..., :3]
    rbg[mask] = 0
    img = Image.fromarray(rbg, mode="RGB")
  img.save(pth)


def create_mesh_video(
    input_dir, output_path, num_frames=480, fps=60
):
  os.makedirs(os.path.dirname(output_path),exist_ok=True)
  first_frame_path = os.path.join(input_dir, "mesh_0000.png")

  if not os.path.exists(first_frame_path):
    raise FileNotFoundError(f"Mesh frame {first_frame_path} not found!")
  ...
  for i in tqdm(range(num_frames)):
    fname = os.path.join(input_dir, f"mesh_{i:04d}.png")
    if not os.path.exists(fname):
      raise FileNotFoundError(f"Mesh frame {fname} not found!")
