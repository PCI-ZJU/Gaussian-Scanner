...
def lpips_metric(network_output, gt):
    with torch.no_grad():
        device = network_output.device
        lpips_fn = lpips.LPIPS(net='vgg').to(device).eval()
        network_output_expected = network_output * 2.0 - 1.0
        gt_expected = gt * 2.0 - 1.0
        lp = lpips_fn(network_output_expected.unsqueeze(0), gt_expected.unsqueeze(0))
    return lp
